            ...
            try:
                ...
                if not hasattr(GemmaForCausalLM, '_vla_compat_patched'):
                    def _get_embed_tokens(self):
                        ...
                        if hasattr(self, '__dict__') and '_cached_embed_tokens' in self.__dict__:
                            return self.__dict__['_cached_embed_tokens']
                        
                        # Try model.embed_tokens (common in newer transformers)
                        if hasattr(self, 'model') and hasattr(self.model, 'embed_tokens'):
                            self.__dict__['_cached_embed_tokens'] = self.model.embed_tokens
                            return self.model.embed_tokens
                        
                        # Try get_input_embeddings() method
                        if hasattr(self, 'get_input_embeddings'):
                            embeddings = self.get_input_embeddings()
                            self.__dict__['_cached_embed_tokens'] = embeddings
                            return embeddings
                        ...
                        for attr_name in ['embeddings', 'word_embeddings']:
                            if hasattr(self, attr_name):
                                ...
                                self.__dict__['_cached_embed_tokens'] = emb
                                return emb
